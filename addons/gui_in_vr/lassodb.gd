...
class PointOfInterest:
	...
	@export var width: float = 0.3  # Half-width of the snapping rectangle
	@export var height: float = 0.3  # Half-height of the snapping rectangle
	@export var center_multiplier: float = 1.0  # Highest priority for center
	@export var edge_multiplier: float = 0.8  # Medium for edges
	@export var corner_multiplier: float = 0.6  # Lowest for corners
...
func calc_top_two_snapping_power(source: Transform3D, current_snap: Node, 
	...
	for pt in point_set:
		...
		if next and next.is_valid_origin() and next.snapping_enabled:
			...
			var rejection_length: float = max(
				abs(point_local.x) / (next.width / 2.0),
				abs(point_local.y) / (next.height / 2.0)
			)

			var zone_multiplier: float = next.corner_multiplier
			var third_width: float = next.width / 3.0
			var third_height: float = next.height / 3.0
			if abs(point_local.x) < third_width and abs(point_local.y) < third_height:
				zone_multiplier = next.center_multiplier
			elif abs(point_local.x) < third_width:
				zone_multiplier = next.edge_multiplier
			elif abs(point_local.y) < third_height:
				zone_multiplier = next.edge_multiplier

			var snapping_power: float = 0
			if rejection_length <= 1.0:
				snapping_power = next.snapping_power / (1.0 + euclidian_dist) / (0.01 + angular_dist) * zone_multiplier
			else:
